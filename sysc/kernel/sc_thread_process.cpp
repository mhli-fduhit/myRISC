...
//    sc_process_b*    active_p = sc_get_current_process_b();
	...
	//DM 08/15/2019
	simc_p->new_waiting_proc.push_back(thread_h);	
...
{ 
    ...
    e->remove_dynamic( this); //DM 08/16/2019
    ...
    */
    ...
    trigger_t init_trigger_type = m_trigger_type; //DM 08/15/2019 the following switch block may modify the trigger_type but the initial trigger type is needed for correctly waking up threads waiting on AND_LIST and OR_LIST
    ...
    else
    {
        ...
        switch( e->m_notify_type )
        {
            ...
            case sc_event::DELTA: // delta notification
                ...
                //15:09 2018/7/8 ZC
                //for event_list
                if(init_trigger_type == AND_LIST || init_trigger_type == OR_LIST ){
...
{ 
    ...
    // printf("sc_thread_process.cpp 1113 event %s notified on time ",e->name());
    // sc_timestamp ts_tmp=e_delivery_time;
    // sc_time t_tmp=ts_tmp.get_time_count();
    // t_tmp.print();
    // printf(",%lld\n",ts_tmp.m_delta_count);   
    //if a thread starts waiting on event e at time 10,
    //and e are all notified before 10, then the thread should
    //not wake up and return false, and can_wake_up is also false
    if( e->m_notify_type == sc_event::DELTA){
