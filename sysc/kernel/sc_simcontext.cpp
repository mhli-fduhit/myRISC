...
#ifndef _SYSC_NUM_INVOKERS_ENV_VAR
#define _SYSC_NUM_INVOKERS_ENV_VAR "SYSC_NUM_INVOKERS"
#endif
...
void Invoker::method_invoker() {
	while(1) {
      ...
      //added for getting the timestamp of the underlying method, ZC 2019,Dec,10
      proc_handle->set_timestamp(current_method->get_timestamp());
		...
		}
		{
			...
			while( !method_queue.empty() ) {
				...
				//DM 07/28/2019 experiment
				if(current_method->m_process_state == 2) 
				{
				    for(std::vector<sc_method_process*>::iterator method_iter =
					((sc_method_process*)current_method)->m_contingent_methods.begin();
					method_iter != ((sc_method_process*)current_method)->m_contingent_methods.end();
					method_iter++)
				    {
					//if((*method_iter)->get_timestamp() < current_method->get_timestamp())
				    		(*method_iter)->dependent_methods.insert((sc_method_process*)current_method);
    				    }
				}
				sc_get_curr_simcontext()->new_waiting_proc.push_back(current_method);
				sc_get_curr_simcontext()->running_methods--;
				sc_get_curr_simcontext()->remove_running_process( current_method );
				//method_group_no = current_method->group_no; //ASSUMES 1to1 METHOD-to-INVOKER MAPPING
				//sc_get_curr_simcontext()->running_groups[method_group_no].erase(current_method);
...
void Invoker::suspend_invoker() {
	...
	if(cur_kernel_requests == 1 && /*simc_p->running_groups[method_group_no].empty()*/simc_p->running_invokers.size() == 0) {
...
unsigned int _SYSC_NUM_INVOKERS;
...
sc_simcontext::sc_simcontext() :
    ...
    running_methods(0),
    num_kernel_requests(0),
    total_runrdycheck(0),total_eventcheck(0),total_waitcheck(0),total_threadevent(0),total_delivery(0),total_eventpred(0)
{
    init();
    pthread_mutex_init( &kernel_request_mutex, NULL );
}
...
void sc_simcontext::add_to_wait_queue(sc_process_b* process_h)
{
    ...
    m_waiting_proc_queue.insert(process_h);
}
...
void sc_simcontext::remove_from_wait_queue(sc_process_b* process_h)
{
    ...
    m_waiting_proc_queue.erase(process_h);
}
...
bool heap_cmp_pair(std::pair<sc_thread_process*,sc_timestamp>& s, std::pair<sc_thread_process*,sc_timestamp>& t)
{
	return s.second > t.second;
}

void
sc_simcontext::update_dependency_set()
{
    // running process handle: NULL if it is the root thread
    sc_process_b* m_process_b = get_curr_proc();

    //DM 07/28/2019 experiment
    if(RCAST<sc_thread_process*>(m_process_b) && !m_process_b->invoker) {
    sc_thread_process* curr_thread = (sc_thread_process*) m_process_b;
/*
    int prev_id = curr_thread->get_instance_id();
    for(auto iter = curr_thread->dependency_checked_segs.begin();
	iter != curr_thread->dependency_checked_segs.end();
	iter++) 
    {
	if(prev_id != iter->second) {
		std::cout << "DM ERROR!\n";
		assert(0);
	}
	prev_id = iter->second;
...
*/
    if(curr_thread->dependency_checked_segs.
	count(curr_thread->get_segment_id() ) == 0) 
    {
	    curr_thread->dependency_checked_segs.
		insert(curr_thread->get_segment_id() );
	    int it1_seg = m_process_b->get_segment_id();
	    int it1_inst = m_process_b->get_instance_id();
	    if(it1_seg != -1 && it1_seg != -2) 
	    {
	    	int _OoO_Prediction_Event_Notification_Table_Size=sqrt(_OoO_Combined_Data_Conflict_Table_Size); 
		int Ncolumns_Data_Conflict_Table =sqrt(_OoO_Combined_Data_Conflict_Table_Size); //this should be changed. 

	    	sc_method_handle method_p2;
		for ( method_p2 = m_process_table->method_q_head(); 
			  method_p2; method_p2 = method_p2->next_exist() )
		{

			int it2_seg = (method_p2)->get_segment_id();
			int it2_inst = (method_p2)->get_instance_id();
	        	...
	        	long long pred_t = _OoO_Prediction_Event_Notification_Table_No_Indirect_Time_Units[
	        	    _OoO_Prediction_Event_Notification_Table_Size*ETP_id2+ETP_id1];
	        	long long pred_d = _OoO_Prediction_Event_Notification_Table_No_Indirect_Delta[
	        	    _OoO_Prediction_Event_Notification_Table_Size*ETP_id2+ETP_id1];
	
	        	if( pred_t!=-1 && pred_d!=-1 )
	        	{
			   (method_p2)->dependent_threads.insert((sc_thread_process*)m_process_b);
			   ((sc_thread_process*)m_process_b)->m_contingent_methods[m_process_b->get_segment_id()].push_back(method_p2);
	        	}
			int CDCT_id = combined_data_conflict_table_index_lookup( it2_seg, it2_inst );
	    		int CDCT_id1 = combined_data_conflict_table_index_lookup( it1_seg, it1_inst );

	    		int table_entry = _OoO_Combined_Data_Conflict_Table[CDCT_id1 * Ncolumns_Data_Conflict_Table + CDCT_id];
	    		if(table_entry > 0) {
	    		method_p2->combined_data_conflict_threads.insert(curr_thread);
	    		}
	    	}
	    }
    }
    ...
    {
    	for( std::vector<sc_method_process*>::iterator method_iter = ((sc_thread_process*)m_process_b)->m_contingent_methods[m_process_b->get_segment_id()].begin();
		method_iter != ((sc_thread_process*)m_process_b)->m_contingent_methods[m_process_b->get_segment_id()].end();
		method_iter++)
	{
		(*method_iter)->dependent_threads.insert((sc_thread_process*)m_process_b);
    	}
    }
	...
	curr_thread->dependent_threads.clear();
	curr_thread->dependent_methods.clear();
	//curr_thread->m_contingent_methods.clear(); TODO
	//curr_thread->m_contingent_threads.clear();
	int it1_seg = (curr_thread)->get_segment_id();
        int it1_inst = (curr_thread)->get_instance_id();
	if(it1_seg != -1 && it1_seg != -2) 
	{
	sc_thread_handle thread_p2;
	sc_method_handle method_p2;

	std::vector<std::pair<sc_thread_process*,sc_timestamp> > indirect_contingent_threads;
	std::vector<std::pair<sc_thread_process*,sc_timestamp> > indirect_dependent_threads;

	for ( thread_p2 = m_process_table->thread_q_head(); 
	  thread_p2; thread_p2 = thread_p2->next_exist() )
    	{
	    int it2_seg = (thread_p2)->get_segment_id();
            int it2_inst = (thread_p2)->get_instance_id();
            if(it2_seg == -1 || it2_seg == -2) continue;
            //do the prediction
	    ...
	    if( pred_t!=-1 && pred_d !=-1)
	    {	
		thread_p2->dependent_threads[thread_p2->get_segment_id()].insert(curr_thread);
	 	//thread_p2->dependent_threads[sc_timestamp(pred_t/res,pred_d)].insert(curr_thread);
		curr_thread->m_contingent_threads[curr_thread->get_segment_id()].insert(thread_p2);
	    }
	       
	    long long reverse_pred_t = _OoO_Prediction_Event_Notification_Table_No_Indirect_Time_Units[
                _OoO_Prediction_Event_Notification_Table_Size*ETP_id1+ETP_id2];
            long long reverse_pred_d = _OoO_Prediction_Event_Notification_Table_No_Indirect_Delta[
                _OoO_Prediction_Event_Notification_Table_Size*ETP_id1+ETP_id2];
	    
	    if( reverse_pred_t!=-1 && reverse_pred_d!=-1 )
	    {
		curr_thread->dependent_threads[curr_thread->get_segment_id()].insert(thread_p2);
		//curr_thread->dependent_threads[sc_timestamp(pred_t/res,pred_d)].insert(thread_p2);
		thread_p2->m_contingent_threads[thread_p2->get_segment_id()].insert(curr_thread);
		//thread_p2->m_contingent_threads[thread_p2->get_segment_id()].insert(curr_thread);
	    }
	    ...
	    int table_entry = _OoO_Combined_Data_Conflict_Table[CDCT_id1 * Ncolumns_Data_Conflict_Table + CDCT_id];
	    if(table_entry > 0) {
	    	thread_p2->combined_data_conflict_threads[thread_p2->get_segment_id()].insert(curr_thread);
	    }
	    int reverse_table_entry = _OoO_Combined_Data_Conflict_Table[CDCT_id * Ncolumns_Data_Conflict_Table + CDCT_id1];
	    if(reverse_table_entry > 0) {
	    	curr_thread->combined_data_conflict_threads[curr_thread->get_segment_id()].insert(thread_p2);
	    }
	...
	for ( method_p2 = m_process_table->method_q_head(); 
	  method_p2; method_p2 = method_p2->next_exist() )
	    ...
	    int it2_seg = (method_p2)->get_segment_id();
            ...
            if( reverse_pred_t!=-1 && reverse_pred_d!=-1 )
            {
		curr_thread->dependent_methods[curr_thread->get_segment_id()].insert(method_p2);
		method_p2->m_contingent_threads.insert(curr_thread);
            }
	    int CDCT_id = combined_data_conflict_table_index_lookup( it2_seg, it2_inst );
	    int CDCT_id1 = combined_data_conflict_table_index_lookup( it1_seg, it1_inst );
	    ...
	    int reverse_table_entry = _OoO_Combined_Data_Conflict_Table[CDCT_id * Ncolumns_Data_Conflict_Table + CDCT_id1];
	    if(reverse_table_entry > 0) {
	    	curr_thread->combined_data_conflict_methods[curr_thread->get_segment_id()].insert(method_p2);
	    }
...
/*
	...
	{
		std::cout << "waiting " << (*it)->name() << " PWT: ";
		if((*it)->possible_wakeup_time.get_infinite()) {
			std::cout << "infinite!\n";
		}
		else {
			(*it)->possible_wakeup_time.show();
		}
		sc_thread_process* m_handle = (sc_thread_process*) (*it);
		for(auto dep_it = m_handle->dependent_threads[m_handle->get_segment_id()].begin();
			dep_it != m_handle->dependent_threads[m_handle->get_segment_id()].end();
			dep_it++)
		{
			std::cout << "\t" << (*dep_it)->name() << "\n";
		}
	}
    }
*/
    ...
    if ( !m_runnable->is_empty() )
    {
        ...
        while ( (!m_runnable->is_empty()
                    && m_curr_proc_queue.size() - running_methods + running_invokers.size() <_SYSC_NUM_INVOKERS )
		    || (!m_runnable->is_empty() && ready_invokers.size() != 0) )
        {
            // execute method processes
            m_runnable->toggle_methods();
            sc_method_handle method_h = pop_runnable_method();
            ...
            if ( method_h != 0 ) 
            {
		bool no_conflict = has_no_conflicts_method( (sc_process_b*)method_h, conflict_methods, conflict_threads );
                // has no conflicts
                if ( no_conflict /*has_no_conflicts_method( (sc_process_b*)method_h, conflict_methods, conflict_threads )*/ )
                {
                		m_curr_proc_queue.push_back( (sc_process_b*)method_h );
					...
					//running_groups[method_h->group_no].insert(method_h);
		...
		bool no_conflict = has_no_conflicts( (sc_process_b*)thread_h, conflict_methods, conflict_threads );
        
                if ( no_conflict /*has_no_conflicts( (sc_process_b*)thread_h, conflict_methods, conflict_threads)*/) 
                {
        ...
        //DM 05/16/2019
        for(std::unordered_set<Invoker*>::iterator invok_iter = ready_invokers.begin();
            ...
            invok_iter = ready_invokers.erase(invok_iter); // (bugfix 2/25/20, RD)
        }
    ...
    if(get_curr_proc() == NULL && verbosity_flag_5)
    {
        ...
        std::cout << "predict wakeup runrdy =  " 
            << total_runrdycheck << " cycles" << std::endl;
	std::cout << "predict wakeup event = " 
            << total_eventcheck << " cycles" << std::endl;
	std::cout << "predict wakeup wait " 
            << total_waitcheck << " cycles" << std::endl;
	std::cout << "stage 4: " 
            << total_threadevent << " cycles" << std::endl;
	std::cout << "delivery: " 
            << total_delivery << " cycles" << std::endl;
	std::cout << "event prediction runs for total of "
	    << total_eventpred << " cycles" << std::endl;
...
void sc_simcontext::predict_wakeup_create_defining_tree(std::unordered_map<sc_process_b*,
        std::unordered_map<sc_event*, sc_timestamp> >& wkup_t_evnt) {

//int _OoO_Prediction_Event_Notification_Table_Size=sqrt(_OoO_Combined_Data_Conflict_Table_Size);

    //std::unordered_set<sc_process_b*> defining_tree;

    for(std::list<sc_process_b*>::iterator new_wait_it = new_waiting_proc.begin();
	new_wait_it != new_waiting_proc.end();
	new_wait_it++)
    {
	for(std::unordered_set<sc_event*>::iterator new_event_it = new_event_notify[*new_wait_it].begin();
		new_event_it != new_event_notify[*new_wait_it].end();
		new_event_it++)
	{
		for(std::vector<sc_thread_handle>::iterator 
        	    it1  = (*new_event_it)->m_threads_dynamic.begin();
        	    it1 != (*new_event_it)->m_threads_dynamic.end();
        	    ++ it1)
        	{
	            sc_timestamp it1_t = (*it1)->get_timestamp();
	            sc_timestamp event_t = (*new_event_it)->get_earliest_time_after_certain_time(it1_t);
		    if(event_t + sc_timestamp(0,1) == (*it1)->possible_wakeup_time) {
	    	    	(*it1)->possible_wakeup_time = event_t + sc_timestamp(0,1);
		    	if( (*it1)->PWT_defining_proc != NULL) {
		    		(*it1)->PWT_defining_proc->proc_defined.erase(*it1);
		    	}
		    	(*it1)->PWT_defining_proc = NULL;
		    	wkup_t_evnt[(sc_process_b*)(*it1)][*new_event_it] = event_t;
	    	    }
        	}
    		for(std::vector<sc_method_handle>::iterator 
        	    it1  = (*new_event_it)->m_methods_dynamic.begin();
        	    it1 != (*new_event_it)->m_methods_dynamic.end();
        	    ++ it1)
        	{
        	    sc_timestamp it1_t = (*it1)->get_timestamp();
        	    sc_timestamp event_t = (*new_event_it)->get_earliest_time_after_certain_time(it1_t);
	...
	new_event_notify[*new_wait_it].clear();
	new_event_notify.erase(*new_wait_it);
    }   
 
    while(!new_waiting_proc.empty())
    {
	
	sc_process_b* proc_it = new_waiting_proc.front();

	for(std::unordered_set<sc_process_b*>::iterator def_it = proc_it->proc_defined.begin();
		def_it != proc_it->proc_defined.end();
		def_it++)
	{
		new_waiting_proc.push_back(*def_it);
	}
	
	proc_it->possible_wakeup_time = sc_timestamp(-1,-1);
	proc_it->min_PWT_distance = sc_timestamp(-1,-1);
	if(proc_it->PWT_defining_proc != NULL) {
		proc_it->PWT_defining_proc->proc_defined.erase(proc_it);
	}
	proc_it->PWT_defining_proc = NULL;
	proc_it->proc_defined.clear();
	defining_tree.insert(proc_it);
	new_waiting_proc.pop_front();
    }

}

//start: helper function for event delivery
void sc_simcontext::predict_wakeup_time_by_running_ready_threads()
{
    int _OoO_Prediction_Event_Notification_Table_Size=sqrt(_OoO_Combined_Data_Conflict_Table_Size);

    for(std::unordered_set<sc_process_b*>::iterator tree_iter = defining_tree.begin();
	tree_iter != defining_tree.end();
	tree_iter++)
    {
	int it1_seg = (*tree_iter)->get_segment_id();
        int it1_inst = (*tree_iter)->get_instance_id();
        if(it1_seg == -1 || it1_seg == -2) continue;

	if((*tree_iter)->m_process_kind == SC_METHOD_PROC_) 
	{
		sc_method_process* m_handle = (sc_method_process*) (*tree_iter);
		for(std::vector<sc_method_process*>::iterator method_it2 = m_handle->m_contingent_methods.begin();
			method_it2 != (m_handle)->m_contingent_methods.end();
			method_it2++)
		{
			int it2_seg = (*method_it2)->get_segment_id();
        		int it2_inst = (*method_it2)->get_instance_id();
        		if(it2_seg == -1 || it2_seg == -2 || (*method_it2)->m_process_state == 4) continue;
			int ETP_id1 = event_prediction_table_index_lookup( it1_seg, it1_inst );
            		int ETP_id2 = event_prediction_table_index_lookup( it2_seg, it2_inst );
            		long long pred_t = _OoO_Prediction_Event_Notification_Table_No_Indirect_Time_Units[
                		_OoO_Prediction_Event_Notification_Table_Size*ETP_id2+ETP_id1];
            		...
            		{
				sc_timestamp it2_t;
                		if((*method_it2)->m_process_state != 2) it2_t = (*method_it2)->get_timestamp();
                		else it2_t = (*method_it2)->possible_wakeup_time;
				sc_timestamp pred_it1_t = 
                    			it2_t + sc_timestamp( pred_t/res,pred_d);
                		if(pred_it1_t < (*tree_iter)->possible_wakeup_time)
				{
					(*tree_iter)->min_PWT_distance = sc_timestamp(pred_t/res,pred_d);
                    			(*tree_iter)->possible_wakeup_time = pred_it1_t;
					if( (*tree_iter)->PWT_defining_proc != NULL) {
						(*tree_iter)->PWT_defining_proc->proc_defined.erase(*tree_iter);
					}
					(*tree_iter)->PWT_defining_proc = (sc_process_b*) (*method_it2);
					(*method_it2)->proc_defined.insert(*tree_iter);
					mod_wakeup_proc.insert(*tree_iter);
					//break;
				}
				else if((pred_it1_t == (*tree_iter)->possible_wakeup_time 
						&& sc_timestamp(pred_t/res,pred_d) < (*tree_iter)->min_PWT_distance))
				{
					(*tree_iter)->min_PWT_distance = sc_timestamp(pred_t/res,pred_d);
					if( (*tree_iter)->PWT_defining_proc != NULL) {
						(*tree_iter)->PWT_defining_proc->proc_defined.erase(*tree_iter);
		...
		for(std::unordered_set<sc_thread_process*>::iterator thread_it2 = m_handle->m_contingent_threads.begin();
			thread_it2 != (m_handle)->m_contingent_threads.end();
			thread_it2++)
		{
			int it2_seg = (*thread_it2)->get_segment_id();
        		int it2_inst = (*thread_it2)->get_instance_id();
        		if(it2_seg == -1 || it2_seg == -2 || (*thread_it2)->m_process_state == 4) continue;
			int ETP_id1 = event_prediction_table_index_lookup( it1_seg, it1_inst );
            		int ETP_id2 = event_prediction_table_index_lookup( it2_seg, it2_inst );
            		long long pred_t = _OoO_Prediction_Event_Notification_Table_No_Indirect_Time_Units[
                		_OoO_Prediction_Event_Notification_Table_Size*ETP_id2+ETP_id1];
            		...
            		{
                		sc_timestamp it2_t;
                		if((*thread_it2)->m_process_state != 2) it2_t = (*thread_it2)->get_timestamp();
                		else it2_t = (*thread_it2)->possible_wakeup_time;
				sc_timestamp pred_it1_t = 
                    			it2_t + sc_timestamp( pred_t/res,pred_d);
                		if(pred_it1_t < (*tree_iter)->possible_wakeup_time)
				{
					...
					(*tree_iter)->PWT_defining_proc = (sc_process_b*) (*thread_it2);
					(*thread_it2)->proc_defined.insert(*tree_iter);
					mod_wakeup_proc.insert(*tree_iter);
					//break;
				}
				else if((pred_it1_t == (*tree_iter)->possible_wakeup_time 
	...
	{
		sc_thread_process* m_handle = (sc_thread_process*) (*tree_iter);
		for(std::vector<sc_method_process*>::iterator method_it2 = m_handle->m_contingent_methods[m_handle->get_segment_id()].begin();
			method_it2 != (m_handle)->m_contingent_methods[m_handle->get_segment_id()].end();
			method_it2++)
		{
			int it2_seg = (*method_it2)->get_segment_id();
        		int it2_inst = (*method_it2)->get_instance_id();
		...
		for(std::unordered_set<sc_thread_process*>::iterator thread_it2 = m_handle->m_contingent_threads[m_handle->get_segment_id()].begin();
			thread_it2 != m_handle->m_contingent_threads[m_handle->get_segment_id()].end();
			thread_it2++)
		{	
			int it2_seg = (*thread_it2)->get_segment_id();
		        int it2_inst = (*thread_it2)->get_instance_id();
        ...
        std::unordered_map<sc_event*, sc_timestamp> >& wkup_t_evnt)
{
    ...
    {   
	    ...
	    if(event_t + sc_timestamp(0,1) < (*it1)->possible_wakeup_time) {
		    ...
		    (*it1)->PWT_defining_proc = NULL;
		    mod_wakeup_proc.insert(*it1);
		    wkup_t_evnt.erase((sc_process_b*)(*it1));
		    wkup_t_evnt[(sc_process_b*)(*it1)][*event_it] = event_t;
		    //defining_tree.erase(*it1);
	    }
	    else if(event_t + sc_timestamp(0,1) == (*it1)->possible_wakeup_time) {
	    	    (*it1)->possible_wakeup_time = event_t + sc_timestamp(0,1);
		    if( (*it1)->PWT_defining_proc != NULL) {
		    	(*it1)->PWT_defining_proc->proc_defined.erase(*it1);
		    }
...
void sc_simcontext::predict_wakeup_time_by_waiting_threads(
    std::unordered_map<sc_process_b*, std::unordered_map<sc_event*, sc_timestamp> >& wkup_t_evnt)
{

    int _OoO_Prediction_Event_Notification_Table_Size=sqrt(_OoO_Combined_Data_Conflict_Table_Size);

    std::vector<sc_process_b*> minheap(mod_wakeup_proc.begin(),mod_wakeup_proc.end());
    std::make_heap(minheap.begin(),minheap.end(),&heap_cmp);
 
    //Stops early if a lot threads never wakes up
    while(!mod_wakeup_proc.empty() && !minheap.front()->possible_wakeup_time.get_infinite())
    {
    	...
    	sc_process_b* unvisited_th = minheap.front();
    	sc_timestamp unvisited_t = minheap.front()->possible_wakeup_time;
	std::pop_heap(minheap.begin(),minheap.end(),&heap_cmp);
	minheap.pop_back();
	mod_wakeup_proc.erase(unvisited_th);
	...
	if( unvisited_th->m_process_kind == SC_METHOD_PROC_ ) 
	{
		sc_method_process* m_handle = RCAST<sc_method_process*>(unvisited_th);
		for(std::unordered_set<sc_method_process*>::iterator method_it1 = m_handle->dependent_methods.begin();
			method_it1 != m_handle->dependent_methods.end();
			method_it1++) 
		{
			if( (*method_it1)->m_process_state != 2) continue;
			sc_process_b* th = *method_it1;
	    		sc_timestamp& t = (*method_it1)->possible_wakeup_time;
				...
				double res = sc_dt::uint64_to_double( m_time_params->time_resolution);
    				...
    				if(pred_th_t < t) {
					t = pred_th_t;
					if(th->PWT_defining_proc != NULL) {
						th->PWT_defining_proc->proc_defined.erase(th);
					}
					unvisited_th->proc_defined.insert(th);
					th->PWT_defining_proc = unvisited_th;
					mod_wakeup_proc.insert(th);
					minheap.push_back(th);
					std::push_heap(minheap.begin(),minheap.end());
				}
				else if((pred_th_t == t 
						&& sc_timestamp(pred_t/res,pred_d) < th->min_PWT_distance) ) {
					if(th->PWT_defining_proc != NULL) {
						th->PWT_defining_proc->proc_defined.erase(th);
					}
					unvisited_th->proc_defined.insert(th);
		...
		for(std::unordered_set<sc_thread_process*>::iterator thread_it1 = m_handle->dependent_threads.begin();
			thread_it1 != m_handle->dependent_threads.end();
			thread_it1++) 
		{
			if( (*thread_it1)->m_process_state != 2) continue;
			sc_process_b* th = *thread_it1;
	    		sc_timestamp& t = (*thread_it1)->possible_wakeup_time;
		...
		sc_thread_process* m_handle = RCAST<sc_thread_process*>(unvisited_th);
		
		for(std::unordered_set<sc_method_process*>::iterator method_it1 = m_handle->dependent_methods[m_handle->get_segment_id()].begin();
			method_it1 != m_handle->dependent_methods[m_handle->get_segment_id()].end();
			method_it1++) 
		{
			if( (*method_it1)->m_process_state != 2) continue;
			sc_process_b* th = *method_it1;
		...
		for(std::unordered_set<sc_thread_process*>::iterator thread_it1 = m_handle->dependent_threads[m_handle->get_segment_id()].begin();
			thread_it1 != m_handle->dependent_threads[m_handle->get_segment_id()].end();
			thread_it1++) 
		{
			if( (*thread_it1)->m_process_state != 2) continue;
			sc_process_b* th = *thread_it1;
...
**********************************************************************/
    ...
    if(m_curr_proc_queue.size() - running_methods  + running_invokers.size() >= _SYSC_NUM_INVOKERS) return false;
    //we first collect the information about the predicted wakeup time by
    //running and ready threads in
    //then we collect when is it waken up by event
    std::unordered_map<sc_process_b*,
        std::unordered_map<sc_event*, sc_timestamp> > wkup_t_evnt;
cycles_t start_eventpred = currentcycles();
    predict_wakeup_create_defining_tree(wkup_t_evnt);
    //then decide the wake up time due to events
    //we need to iterate over all the events
cycles_t start_eventcheck = currentcycles();
    predict_wakeup_time_by_events(wkup_t_evnt);
total_eventcheck += currentcycles() - start_eventcheck;


cycles_t start_runrdycheck = currentcycles();
    predict_wakeup_time_by_running_ready_threads();
total_runrdycheck += currentcycles() - start_runrdycheck;
    //now, summerize the earliest time a waiting thread may wakeup
    //it is the min of evnt and pred
cycles_t start_waitcheck = currentcycles();
    predict_wakeup_time_by_waiting_threads(wkup_t_evnt);
total_waitcheck += currentcycles() - start_waitcheck;
total_eventpred += currentcycles() - start_eventpred;
    //The next thing is to check if the earlist time is smaller than the event wake up time for the thread. If yes, then it is not safe to move the thread from the wait queue. Otherwise, we can wakeup the thread and set its time to event wakeup time collected in wkup_t_evnt.
    ...
    std::unordered_map<sc_event*, std::unordered_set<sc_method_process*> > delivered_e_methods;
    std::unordered_map<sc_event*, std::unordered_set<sc_thread_process*> > delivered_e_threads;

cycles_t start_threadevent = currentcycles();
    for(std::unordered_map<sc_process_b*, std::unordered_map<sc_event*, sc_timestamp> >::iterator
        p_iter1  = wkup_t_evnt.begin();
        p_iter1 != wkup_t_evnt.end();
        ++p_iter1)
    {
        sc_process_b* th1 = p_iter1->first;
        //get the predicted wakeup time of th1
        sc_timestamp t1 = p_iter1->first->possible_wakeup_time;
	bool cur_thread_wakeup = false;
	bool cur_thread_e_deliver = false;
...
/*
	    std::unordered_set<sc_event*> visited_e;
            while(1)
            {
		//get the event that wakes up th1
                sc_event* e = NULL;
                //get the time that e wakes up th1
                sc_timestamp t2 = sc_timestamp(-1,-1);;


		for(std::unordered_map<sc_event*, sc_timestamp>::iterator
                	e_iter  = wkup_t_evnt[th1].begin();
                	e_iter != wkup_t_evnt[th1].end();
                	++e_iter) 
		{
			if(visited_e.count(e_iter->first)==0 && e_iter->second < t2) {
				e = e_iter->first;
				t2 = e_iter->second;
			}
		}
...
*/
		...
		if(t2 < t1)
                {
		    cur_thread_e_deliver = true;
                    has_event_delivered = true;
		    bool flag = false;
		    cycles_t start_delivery = currentcycles();
                    if(dynamic_cast<sc_thread_process*>(th1) != NULL){ 
                        flag = dynamic_cast<sc_thread_process*>(th1)->deliver_event_at_time(e, t2);
			if( delivered_e_threads.count(e) == 0) {
				delivered_e_threads[e] = std::unordered_set<sc_thread_process*>({});
				delivered_e_threads[e].insert( (sc_thread_process*) th1);
			}
			else {
				delivered_e_threads[e].insert( (sc_thread_process*) th1);
			}
                    ...
                    {
                        flag = dynamic_cast<sc_method_process*>(th1)->deliver_event_at_time(e, t2);
                        if( delivered_e_methods.count(e) == 0) {
				delivered_e_methods[e] = std::unordered_set<sc_method_process*>({});
				delivered_e_methods[e].insert( (sc_method_process*) th1);
			}
			else {
				delivered_e_methods[e].insert( (sc_method_process*) th1);
			}
		    }
		    total_delivery += currentcycles() - start_delivery;
	    ...
	    if (!cur_thread_wakeup && cur_thread_e_deliver)
	    {
		    	new_waiting_proc.push_back(th1);
	    }
    }
...
/*
    for(std::unordered_map<sc_event*, std::unordered_set<sc_thread_process*> >::iterator e_iter 
	= delivered_e_threads.begin();
	e_iter != delivered_e_threads.end();
	e_iter++ )
    {
	unsigned int init_size = e_iter->first->m_threads_dynamic.size();
	unsigned int cur_size = init_size;
	if(cur_size > 0) {
	for(unsigned int i = 1; i < cur_size;i++)
        {
	   while( e_iter->second.count(e_iter->first->m_threads_dynamic[i]) != 0 && cur_size > 0) {
	   	e_iter->first->m_threads_dynamic[i] = e_iter->first->m_threads_dynamic[cur_size-1];
		cur_size--;
	   }
	}
	
	if( e_iter->second.count(e_iter->first->m_threads_dynamic[0]) != 0 && cur_size > 0) {
	   	e_iter->first->m_threads_dynamic[0] = e_iter->first->m_threads_dynamic[cur_size-1];
		cur_size--;
	}
	e_iter->first->m_threads_dynamic.resize(cur_size);
	}
    }
    for(std::unordered_map<sc_event*, std::unordered_set<sc_method_process*> >::iterator e_iter 
	= delivered_e_methods.begin();
	e_iter != delivered_e_methods.end();
	e_iter++ )
    {
	unsigned int init_size = e_iter->first->m_methods_dynamic.size();
	unsigned int cur_size = init_size;
	if(cur_size > 0) {
	for(unsigned int i = 1; i < cur_size;i++)
        {
	   while( e_iter->second.count(e_iter->first->m_methods_dynamic[i]) != 0 && cur_size > 0) {
	   	e_iter->first->m_methods_dynamic[i] = e_iter->first->m_methods_dynamic[cur_size-1];
		cur_size--;
	   }
	}
	
	if( e_iter->second.count(e_iter->first->m_methods_dynamic[0]) != 0 && cur_size > 0) {
	   	e_iter->first->m_methods_dynamic[0] = e_iter->first->m_methods_dynamic[cur_size-1];
		cur_size--;
	}
	e_iter->first->m_methods_dynamic.resize(cur_size);
	}
    }
*/
total_threadevent += currentcycles() - start_threadevent;
        ...
        std::list<sc_thread_handle> conflict_threads )
{
    ...
    sc_thread_process* m_handle = (sc_thread_process*) process_h;
    std::list<sc_process_b*> concurrent_processes;
if(prediction_switch) {
    for(std::unordered_set<sc_method_process*>::iterator method_it = 
	m_handle->combined_data_conflict_methods[m_handle->get_segment_id()].begin();
	method_it != m_handle->combined_data_conflict_methods[m_handle->get_segment_id()].end();
	method_it++)
    {
	if( (*method_it)->get_timestamp() < ts && (*method_it)->m_process_state!=2 && (*method_it)->m_process_state!=4) {
		concurrent_processes.push_back( (sc_process_b*)(*method_it) );
	}
	else if( (*method_it)->possible_wakeup_time < ts && (*method_it)->m_process_state == 2 ) {
		concurrent_processes.push_back( (sc_process_b*)(*method_it) );
	}
	else if( (*method_it)->get_timestamp() == ts && (*method_it)->m_process_state == 0) {
		concurrent_processes.push_back( (sc_process_b*)(*method_it) );
	}
    }
    for(std::unordered_set<sc_thread_process*>::iterator thread_it = 
	m_handle->combined_data_conflict_threads[m_handle->get_segment_id()].begin();
	thread_it != m_handle->combined_data_conflict_threads[m_handle->get_segment_id()].end();
	thread_it++)
    {
/*	
	std::cout << m_handle->name() << " has combined conflict with " << (*thread_it)->name() << "\n";
	std::cout << (*thread_it)->name() << "PWT: ";
	(*thread_it)->possible_wakeup_time.show();
*/	
    	if( (*thread_it)->get_timestamp() < ts && (*thread_it)->m_process_state!=2 && (*thread_it)->m_process_state != 4) {
		concurrent_processes.push_back( (sc_process_b*)(*thread_it) );
	}
	else if( (*thread_it)->possible_wakeup_time < ts && (*thread_it)->m_process_state == 2) {
		concurrent_processes.push_back( (sc_process_b*)(*thread_it) );
	}
	else if( (*thread_it)->get_timestamp() == ts && (*thread_it)->m_process_state == 0) {
		concurrent_processes.push_back( (sc_process_b*)(*thread_it) );
	}
...
bool
sc_simcontext::has_no_conflicts_method( sc_process_b* process_h,
		std::list<sc_method_handle> conflict_methods,
		std::list<sc_thread_handle> conflict_threads )
{
    // assume we have acquired the kernel lock upon here
    ...
    // the local timestamp of the target process
    sc_timestamp ts = process_h->get_timestamp();
    sc_method_process* m_handle = (sc_method_process*) process_h;
    std::list<sc_process_b*> concurrent_processes;
if(prediction_switch) {
    for(std::unordered_set<sc_method_process*>::iterator method_it = 
	m_handle->combined_data_conflict_methods.begin();
	method_it != m_handle->combined_data_conflict_methods.end();
	method_it++)
    {
	if( (*method_it)->get_timestamp() < ts && (*method_it)->m_process_state!=2 && (*method_it)->m_process_state!=4) {
		concurrent_processes.push_back( (sc_process_b*)(*method_it) );
    ...
    for(std::unordered_set<sc_thread_process*>::iterator thread_it = 
	m_handle->combined_data_conflict_threads.begin();
	thread_it != m_handle->combined_data_conflict_threads.end();
	thread_it++)
    {
    	if( (*thread_it)->get_timestamp() < ts && (*thread_it)->m_process_state!=2 && (*thread_it)->m_process_state!=4) {
		concurrent_processes.push_back( (sc_process_b*)(*thread_it) );
...
sc_simcontext::event_prediction_table_index_lookup( int seg_id, int inst_id )
{
    ...
    int i= inst_id*_OoO_Prediction_Event_Notification_Table_No_Indirect_Number_Segments+seg_id;
    
    return _OoO_Prediction_Event_Notification_No_Indirect_Lookup_Table[i];
...
{
if (  _SYSC_SYNC_PAR_SIM==true ){ //DM 08/16/2019 
    sc_time proc_time;
...
sc_simcontext::prepare_to_simulate()
{
	...
	method_p->possible_wakeup_time = sc_timestamp(-1,-1);
    }
	...
	thread_p->possible_wakeup_time = sc_timestamp(-1,-1);
    }
    ...
    //DM 07/28/2019 experiment
    sc_method_handle  method_p1;  // Pointer to method process accessing.
    sc_method_handle  method_p2;  // Pointer to method process accessing.
    sc_thread_handle  thread_p2;
    int _OoO_Prediction_Event_Notification_Table_Size=sqrt(_OoO_Combined_Data_Conflict_Table_Size);
    int Ncolumns_Data_Conflict_Table =sqrt(_OoO_Combined_Data_Conflict_Table_Size); //this should be changed. 
    for ( method_p1 = m_process_table->method_q_head(); 
	  method_p1; method_p1 = method_p1->next_exist() )
    {
	int it1_seg = (method_p1)->get_segment_id();
        int it1_inst = (method_p1)->get_instance_id();
        if(it1_seg == -1 || it1_seg == -2) continue;
	...
	for ( method_p2 = m_process_table->method_q_head(); 
	  method_p2; method_p2 = method_p2->next_exist() )
	    ...
	    int it2_seg = (method_p2)->get_segment_id();
            ...
            if( pred_t!=-1 && pred_d!=-1 )
            {
		//method_p2->dependent_methods.insert(method_p1);
		method_p1->m_contingent_methods.push_back(method_p2);
            }
	    int CDCT_id = combined_data_conflict_table_index_lookup( it2_seg, it2_inst );
	    int CDCT_id1 = combined_data_conflict_table_index_lookup( it1_seg, it1_inst );
	    ...
	    int table_entry = _OoO_Combined_Data_Conflict_Table[CDCT_id1 * Ncolumns_Data_Conflict_Table + CDCT_id];
	    if(table_entry > 0) {
	    	method_p2->combined_data_conflict_methods.insert(method_p1);
	    }
	...
	for ( thread_p2 = m_process_table->thread_q_head(); 
	  thread_p2; thread_p2 = thread_p2->next_exist() )
	    ...
	    int it2_seg = (thread_p2)->get_segment_id();
            ...
            if( pred_t!=-1 && pred_d!=-1 )
            {
		//thread_p2->dependent_methods.insert(method_p1);
		method_p1->m_contingent_threads.insert(thread_p2);
            }
	    ...
	    int table_entry = _OoO_Combined_Data_Conflict_Table[CDCT_id1 * Ncolumns_Data_Conflict_Table + CDCT_id];
	    if(table_entry > 0) {
	    	thread_p2->combined_data_conflict_methods[thread_p2->get_segment_id()].insert(method_p1);
	    }
	
    	}
    }
    sc_thread_handle thread_p1;
    for ( thread_p1 = m_process_table->thread_q_head(); 
	  thread_p1; thread_p1 = thread_p1->next_exist() )
    {
	int it1_seg = (thread_p1)->get_segment_id();
        int it1_inst = (thread_p1)->get_instance_id();
        if(it1_seg == -1 || it1_seg == -2) continue;

	//thread_p1->dependency_checked_segs.insert(std::make_pair(it1_seg,it1_inst));
	thread_p1->dependency_checked_segs.insert(it1_seg);
	...
	for ( method_p2 = m_process_table->method_q_head(); 
	  method_p2; method_p2 = method_p2->next_exist() )
	    ...
	    int it2_seg = (method_p2)->get_segment_id();
            ...
            if( pred_t!=-1 && pred_d!=-1 )
            {
		//method_p2->dependent_threads.insert(thread_p1);
            	thread_p1->m_contingent_methods[thread_p1->get_segment_id()].push_back(method_p2);
	    }
	    ...
	    int table_entry = _OoO_Combined_Data_Conflict_Table[CDCT_id1 * Ncolumns_Data_Conflict_Table + CDCT_id];
	    if(table_entry > 0) {
	    	method_p2->combined_data_conflict_threads.insert(thread_p1);
	    }
    	}
	...
	for ( thread_p2 = m_process_table->thread_q_head(); 
	  thread_p2; thread_p2 = thread_p2->next_exist() )
	    ...
	    if( pred_t!=-1 && pred_d!=-1 )
            {
		thread_p2->dependent_threads[thread_p2->get_segment_id()].insert(thread_p1);
		//thread_p2->dependent_threads[sc_timestamp(pred_t/res,pred_d)].insert(thread_p1);
		thread_p1->m_contingent_threads[thread_p1->get_segment_id()].insert(thread_p2);
            }
	    ...
	    int table_entry = _OoO_Combined_Data_Conflict_Table[CDCT_id1 * Ncolumns_Data_Conflict_Table + CDCT_id];
	    if(table_entry > 0) {
	    	thread_p2->combined_data_conflict_threads[thread_p2->get_segment_id()].insert(thread_p1);
	    }
...
/*
    for ( method_p1 = m_process_table->method_q_head(); 
	  method_p1; method_p1 = method_p1->next_exist() )
    {
	if(proc_groups.empty())
	{
		proc_groups.push_back({method_p1});
		method_p1->group_no = 0;
	}
	else
	{
		bool found = false;
		for(unsigned int p_iter = 0; p_iter < proc_groups.size(); p_iter++)
		{
			int no_conflict_count = 0;
			for(std::unordered_set<sc_process_b*>::iterator cur_it = proc_groups[p_iter].begin();
				cur_it != proc_groups[p_iter].end();
				cur_it++)
			{
				if((*cur_it)->m_process_kind == SC_METHOD_PROC_)
				{
					sc_method_process* m_handle = (sc_method_process*) (*cur_it);
					if(m_handle->combined_data_conflict_methods.count(method_p1) == 0)
					{
						//proc_groups[p_iter].insert(method_p1);
						no_conflict_count++;
						//found = true;
						//break;
					}
				...
				{
					sc_thread_process* m_handle = (sc_thread_process*) (*cur_it);
					if(m_handle->combined_data_conflict_methods[m_handle->get_segment_id()].count(method_p1) == 0)
					{
						//proc_groups[p_iter].insert(method_p1);
						no_conflict_count++;
						//found = true;
			...
			if(no_conflict_count == proc_groups[p_iter].size())
			{
				found = true;
				proc_groups[p_iter].insert(method_p1);
				method_p1->group_no = p_iter;
				break;
			}
		}
		if(!found) 
		{
			proc_groups.push_back({method_p1});
			method_p1->group_no = proc_groups.size()-1;
		}
	}
    }

    for(int group_iter = 0; group_iter < proc_groups.size(); group_iter++)
    {
	running_groups.push_back({});
	std::cout << "group no " << group_iter << "\n";
	for(auto proc_it = proc_groups[group_iter].begin();
		proc_it != proc_groups[group_iter].end();
		proc_it++)
	{
		std::cout << "\t" << (*proc_it)->name() << "\n";
	}
    }
*/
	     ...
	     method_p->dont_initialize() ) 
	{
		...
		for(std::vector<sc_method_process*>::iterator method_iter = method_p->m_contingent_methods.begin();
			method_iter != method_p->m_contingent_methods.end();
			method_iter++)
		{
			(*method_iter)->dependent_methods.insert(method_p);
		}
		for(std::unordered_set<sc_thread_process*>::iterator thread_iter = method_p->m_contingent_threads.begin();
			thread_iter != method_p->m_contingent_threads.end();
			thread_iter++)
		{
			(*thread_iter)->dependent_methods[(*thread_iter)->get_segment_id()].insert(method_p);
		}
	    }
	     ...
	     thread_p->dont_initialize() ) 
	{
		...
		for(std::vector<sc_method_process*>::iterator method_iter = thread_p->m_contingent_methods[thread_p->get_segment_id()].begin();
			method_iter != thread_p->m_contingent_methods[thread_p->get_segment_id()].end();
			method_iter++)
		{
			(*method_iter)->dependent_threads.insert(thread_p);
		}
		
		for(std::unordered_set<sc_thread_process*>::iterator thread_iter = thread_p->m_contingent_threads[thread_p->get_segment_id()].begin();
			thread_iter != thread_p->m_contingent_threads[thread_p->get_segment_id()].end();
			thread_iter++)
		{
			(*thread_iter)->dependent_threads[(*thread_iter)->get_segment_id()].insert(thread_p);
		}
...
void set_number_sim_cpus()
{
    ...
    const char* sim_invokers_var = _SYSC_NUM_INVOKERS_ENV_VAR;
    char* sim_invokers_str;

    sim_invokers_str = getenv( sim_invokers_var );
    //std::cout << sim_invokers_str  << "xxxx" << std::endl;
    if ( sim_invokers_str )
    {
      
        // 04/06/2015 GL: why not using atoi()? Because it does not handle 
        //                errors (out-of-range?!) properly
        unsigned int len = strlen( sim_invokers_str );
        unsigned int i = 0;
        _SYSC_NUM_INVOKERS = 0;
        while ( len )
        {
            if ( sim_invokers_str[i] >= 0x30 && sim_invokers_str[i] <= 0x39 )
            {
                _SYSC_NUM_INVOKERS = _SYSC_NUM_INVOKERS * 10 + sim_invokers_str[i]
                                      - 0x30;
                len --;
                i ++;
            }
            ...
            {
#ifdef _SYSC_DEFAULT_PAR_SIM_CPUS
                _SYSC_NUM_INVOKERS = _SYSC_DEFAULT_PAR_SIM_CPUS;
#else
                _SYSC_NUM_INVOKERS = 64;
#endif
                break;
            }
        }
...
init = true;
set_number_sim_cpus();
	for(unsigned int i = 0; i < _SYSC_NUM_INVOKERS; i++) {
		std::stringstream invok_strstr;
		invok_strstr << "invoker" << i;
...
*/
	...
	//method_to_invoker_map[ (sc_process_b*)handle ] = m_invokers[ m_invokers.size()-1];
    	...
    	method_to_invoker_map[ (sc_process_b*)handle ] = m_invokers[ method_count % _SYSC_NUM_INVOKERS ];
	method_count++;
