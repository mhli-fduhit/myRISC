/*****************************************************************************
  ...
  License at http://www.accellera.org/. Software distribu ted by Contributors
...
public:
    ...
    //DM 08/11/2019 update dependency set
...
private:
    ...
    void predict_wakeup_create_defining_tree(
	std::unordered_map<sc_process_b*, std::unordered_map<sc_event*, sc_timestamp> >& wkup_t_evnt);
    void predict_wakeup_time_by_running_ready_threads();
    void predict_wakeup_time_by_events(
	std::unordered_map<sc_process_b*, std::unordered_map<sc_event*, sc_timestamp> >& wkup_t_evnt);
...
//	std::unordered_map<sc_process_b*, sc_timestamp>& wkup_t_prd_run_rdy);
    ...
    bool has_no_conflicts_method( sc_process_b*, std::list<sc_method_handle>, std::list<sc_thread_handle> );
...
private:
  ...
  //std::list<sc_process_b*>    m_waiting_proc_queue; //DM 08/16/2019 to unordered_set
  std::unordered_set<sc_process_b*> m_waiting_proc_queue;
...
std::unordered_map<sc_process_b*,Invoker*> method_to_invoker_map;
std::vector<Invoker*> m_invokers;
std::unordered_set<Invoker*> ready_invokers;
std::unordered_set<Invoker*> running_invokers;
...
int num_kernel_requests;
pthread_mutex_t kernel_request_mutex;
//DM 08/14/2019 experiment
std::list<sc_process_b*> new_waiting_proc;
std::unordered_set<sc_process_b*> mod_wakeup_proc;
std::unordered_set<sc_process_b*> defining_tree;
unsigned long long total_runrdycheck;
unsigned long long total_eventcheck;
unsigned long long total_waitcheck;
unsigned long long total_threadevent;
unsigned long long total_delivery;
unsigned long long total_eventpred;
std::unordered_map<sc_process_b*,std::unordered_set<sc_event*> > new_event_notify;
//std::vector<std::unordered_set<sc_process_b*> > running_groups;
//std::vector<std::unordered_set<sc_process_b*> > proc_groups;
};
