...
{
    //DM 07/29/2019 experiment
    for(std::vector<sc_method_process*>::iterator method_iter =
	method_h->m_contingent_methods.begin();
	method_iter != method_h->m_contingent_methods.end();
	method_iter++)
    {
    	(*method_iter)->dependent_methods.erase(method_h);
    }
    method_h->PWT_defining_proc = NULL;
...
sc_simcontext::push_runnable_thread( sc_thread_handle thread_h )
{
    //DM 07/29/2019 experiment
    for(std::vector<sc_method_process*>::iterator method_iter =
	thread_h->m_contingent_methods[thread_h->get_segment_id()].begin();
	method_iter != thread_h->m_contingent_methods[thread_h->get_segment_id()].end();
	method_iter++)
    {
    	(*method_iter)->dependent_threads.erase(thread_h);
    }
    thread_h->PWT_defining_proc = NULL;
